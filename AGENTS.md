...
2. **★ 本地双击即用与 file:// 协议纯净兼容（Double-Click & File Protocol Ready）**：
   - **普通用户零门槛保证**：普通用户完全不跑任何服务脚本、直接在电脑文件管理器中双击 `index.html`，必须 100% 能够正常使用所有功能！
...
### 1. 当用户要求【新建或修改小工具】时：
普通用户仅需输入极简自然语言指令（例如：“帮我新建一个小工具：【工具名称与功能需求】”），AI 必须无条件严格遵照本宪法所有标准，一步到位完成以下闭环操作，无需用户在提示词中重复冗余规则：
...
### 3. 应用展示、双视图与管理规则：
- **双视图模式切换（大卡片 ⇋ 手机 App 小图标）**：
  - **详细大卡片视图（Card View）**：展示大尺寸图标、应用标题、两行功能描述、启动入口与操作按钮，适合少量应用精细浏览；
  - **手机 App 小图标视图（Grid Icon View）**：类似 iOS 主屏幕与 Launchpad，高密度栅格排布，圆角独立大图标，下方紧凑居中展示应用名，鼠标悬浮呈现微型操作，适合应用数量增多后的高效率桌面启动；
  - 视图模式通过 `localStorage.getItem('pwabox_view_mode')` 持久化记忆。
- **界面纯粹极简，拒绝注意力干扰**：大厅界面不出现任何多余的说明性横幅或说教文字，示例删除、AI 扩充等说明由 AI 在对话中直接告知用户，让界面回归纯净；
- **AI 接管默认排序**：代码中 `INITIAL_APPS` 的书写顺序即为唯一展示基准，新建工具默认置顶最新，用户指挥 AI 即可随意重排；
- **⭐️ 置顶快捷标记**：用户可在前端点击卡片或图标上的星标，将常用工具临时固定在大厅最前；
...
> **🎯 核心设计五大标尺（The 5 Golden Rules）—— 任何 AI 生成微应用必须 100% 达标：**
> 1. 📱 **双模响应**：移动端 100dvh 满屏沉浸；桌面/4K 端强制双栏专业工作台（8:4 或 7:5），消除视觉下坠；
> 2. 📐 **8pt 间距**：所有内边距、外边距、组件间距严格遵循 4/8pt 网格阶梯（p-2, p-4, p-6...）；
> 3. 🎨 **克制单色**：全屏仅允许 1 个品牌主重音色（Primary Accent），90% 使用中性灰阶，拒绝五彩斑斓；
> 4. ⭕️ **同心圆角**：严格遵循外大内小的几何嵌套律（弹窗 3xl > 面板 2xl > 按钮 xl > 徽章 lg）；
> 5. 🚫 **零滚动条**：单功能轻量应用在手机端必须单屏锁死，绝对严禁出现整页纵向滚动条或上下晃动！
