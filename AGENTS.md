...
2. **挂载到主大厅**：在根目录 `index.html` 的 `APPS` 数据列表中登记新工具卡片。
3. **★ 贴心本地启动与局域网提示**：
...
AI 必须一次性完成以下清理：
1. **删除文件目录**：删除 `apps/<tool-name>/` 整个目录。
2. **解除大厅登记**：从根目录 `index.html` 的 `INITIAL_APPS` 数组中移除该工具条目。
3. **保持零残留**：检查并确保没有断链的引用。

### 3. 应用置顶、排序与隐藏规则：
- **用户自由排序**：用户拖拽卡片与星标置顶保存在本地 `localStorage`，大厅以用户意愿为绝对最高优先。
- **纯前端隐藏**：支持用户在不想看到某个示例应用时直接点击卡片上的 `👁️ 隐藏`，并在底端提供轻量恢复入口。
- **搜索与全局快捷键**：支持敲击 `/` 或 `⌘K` / `Ctrl+K` 快速唤起搜索，按 `Esc` 退出。
...
5. **深浅色主题独立控制（系统联动 + 手动覆盖，且状态显性化）**：
   - **默认行为**：所有微应用及导航大厅，**默认必须跟随系统外观**（`prefers-color-scheme: dark`），且必须监听系统外观变化（实时跟随早晚自动切换）。
   - **★ 状态显性化（清晰区分“自动”与“手动”）**：
     - 严禁让用户猜测当前是自动还是手动！
     - 外观切换控件必须清晰反映当前模式：
   ...
   - **数据隔离保存**：各自保存在本地 `localStorage.getItem('pwabox_<tool-name>_theme')`，互不干扰。
